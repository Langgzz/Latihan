...
class HrAttendanceCorrection(models.Model):
    ...
    # New Data (Correction)
    check_in_new = fields.Datetime(string='New Check In', required=True, readonly=True, states={'draft': [('readonly', False)]})
    ...
    def action_submit(self):
        for rec in self:
            # Validasi New Check In wajib diisi
            if not rec.check_in_new:
                raise UserError(_("New Check In time is required."))
            
            # Validasi Reason wajib diisi dan tidak boleh kosong atau 'auto_entry'
            if not rec.reason or rec.reason == 'auto_entry':
                raise UserError(_("Alasan koreksi wajib diisi! Silakan pilih alasan yang sesuai sebelum submit."))
